#!/usr/bin/env python3
"""
Fixed WhatsApp Watcher for Silver Tier AI Employee System
Monitors WhatsApp for keywords and creates task files
Uses Playwright for browser automation, with fallback to mock implementation
"""
...
"""
    ...
    def scan_whatsapp_messages(self):
        ...
        try:
            ...
            # Try launching with different options to handle potential library issues
            try:
                # Try Firefox first as it might have fewer system dependencies
                self.browser = self.playwright.firefox.launch(headless=False)  # Changed to headless=False so you can see it
                print("Launched Firefox browser successfully")
            except Exception as firefox_err:
                print(f"Firefox launch failed: {firefox_err}")
                print("Trying Chromium...")

                # If Firefox fails, try Chromium with additional options
                self.browser = self.playwright.chromium.launch(
                    headless=False,  # Changed to False so you can see the QR code
                    args=['--no-sandbox', '--disable-dev-shm-usage']
                )
                print("Launched Chromium browser successfully")
            ...
            self.page.wait_for_selector('#pane-side', timeout=120000)  # Wait up to 120 seconds
            ...
            print("Using mock implementation due to browser error...")
            # Fall back to mock implementation if browser fails
            self.scan_messages_mock()
            log_action("whatsapp_scan", "whatsapp_watcher", "auto", f"failed: {str(e)}, using mock")
        finally:
            # Clean up resources
            if self.browser:
                try:
...
if __name__ == "__main__":
    print("Starting WhatsApp Watcher with improved error handling...")
